...
> Runs on top of `brand.md` (the soul). The soul decides _how WHOLESIP sounds_; this file decides _what this job is_. Your server loads `brand.md` first, then the product's form fields (including the product description, the richest input), then appends this task.

## The job

Write the two SEO fields for a single WHOLESIP product: a **SEO title** and a **SEO meta description**. These are what show up in Google search results and browser tabs — so they must be clear and findable first, and unmistakably WHOLESIP second. They are NOT the storefront description; they are short, factual, search-facing copy.
...
- You may end with the brand using a separator, e.g. `Almond Milk — Stone-Crushed, Whole | WHOLESIP`, but only if it still fits comfortably under 60 characters. If it won't fit, drop the brand, never the searchable term.
...
- One or two calm sentences. Say what it is and why it's real/different, in WHOLESIP's voice — warm, honest, never pushy.
...
SKU name, which can follow), plain and searchable, optionally "| WHOLESIP" if it
still fits, no hype or caps. seo_description: aim for 140–160 characters (stay
under 160), one or two calm sentences in WHOLESIP's voice saying what it is and
...
- Is the description warm, WHOLESIP, and clearly different from the title — with zero hype, urgency, or invented claims?
